...
function buildSnapshot(){
  return {v:5,updatedAt:Date.now(),
    ...
    active:STATE.active,closedSignals:STATE.closedSignals,pending:STATE.pending,
...
function restoreSnapshot(snap){
  ...
  try{
    ...
    if(Array.isArray(snap.active))STATE.active=snap.active;
    if(Array.isArray(snap.closedSignals))STATE.closedSignals=snap.closedSignals;
    if(snap.pending&&typeof snap.pending==="object")STATE.pending=snap.pending;
...
/* =========================================================================
   USDT DOMINANCE (لایو) — از CoinGecko /global (رایگان، بدون کلید، CORS باز).
   جهت از رگرسیون روی تاریخچهٔ نمونه‌ها ساخته می‌شود (نه مقایسهٔ تک‌نقطه‌ای)،
   تا با نویزِ یک نمونه جهت عوض نشود. اگر CoinGecko در دسترس نبود، پنل به
   پروکسی BTC برمی‌گردد و صادقانه «در دسترس نیست» نشان می‌دهد.
   ========================================================================= */
...
async function fetchUsdtDominance(){
  const st=domLoad();const now=Date.now();
  const last=st.samples[st.samples.length-1];
  // بیشتر از هر ۲ دقیقه یک‌بار نمونه نگیر (rate-limit رایگان CoinGecko)
  ...
  try{
    ...
    // خرابی شبکه/فیلتر: با نمونه‌های قدیمی (اگر تازه باشند) ادامه بده، وگرنه unavailable
    const fresh=last&&(now-last.t)<30*60000;
    return fresh?domState(st,true):domState(st,false);
  }
}
function domState(st,available){
  const s=st.samples;
  if(!available||s.length===0)return{available:false,value:null,dir:"UNKNOWN",slopePerHr:0,samples:s.length};
  const value=s[s.length-1].v;
  if(s.length<3)return{available:true,value,dir:"FLAT",slopePerHr:0,samples:s.length};
  // رگرسیون خطی روی نمونه‌های اخیر → شیب برحسب واحدِ درصد در ساعت
  const win=s.slice(-12);
  const t0=win[0].t;const pts=win.map(p=>[(p.t-t0)/3600000,p.v]);
  const f=lineFit(pts);const slope=f.slope;
  let dir="FLAT";
  ...
  return{available:true,value,dir,slopePerHr:slope,samples:s.length};
}
function domConflicts(r,dom){
  // USDT.D صعودی = پول به تتر = فشار روی آلت‌ها → LONGِ آلت نگه داشته شود
  // USDT.D نزولی = ریسک‌آن → SHORTِ آلت (در حالت قوی) نگه داشته شود
  if(!dom||!dom.available)return false;
  if(r.binance_symbol==="BTCUSDT")return false;
  if(typeof COMMODITY_KEEP!=="undefined"&&COMMODITY_KEEP.has(r.asset_symbol))return false;
  if(r.direction==="LONG")return dom.dir==="UP"||dom.dir==="UP_STRONG";
  if(r.direction==="SHORT")return dom.dir==="DOWN_STRONG";
  return false;
}
function domFa(dom){
  if(!dom||!dom.available)return"USDT.D: در دسترس نیست";
  const arrow={UP_STRONG:"▲▲",UP:"▲",FLAT:"◆",DOWN:"▼",DOWN_STRONG:"▼▼"}[dom.dir]||"◆";
  return`USDT.D ${dom.value.toFixed(2)}% ${arrow}`;
}
...
async function fetchMarketContext(){
  ...
  try{
    ...
    base={risk_mode,btc_dir:m.direction,btc_change_24h:chg};
  }catch(e){base={risk_mode:"UNKNOWN"};}
  try{base.usdt_dom=await fetchUsdtDominance();}catch(e){base.usdt_dom={available:false,dir:"UNKNOWN",value:null};}
  // مهم: risk_mode (ورودی امتیاز موتور) دست نمی‌خورد — دامیننس فقط در لایهٔ پایداری/نمایش اثر دارد.
  return base;
}
...
/* =========================================================================
   STATE + SCAN
   ========================================================================= */
const STATE={signals:[],watchlist:[],universe:[],active:[],closedSignals:[],pending:{},market_context:{},last_scan_at:null,scanning:false};
...
/* =========================================================================
   SIGNAL STABILITY LAYER — «ورود سخت، خروج چسبنده»
   موتور و آستانه‌های قفل‌شده دست‌نخورده‌اند؛ این لایه فقط تعیین می‌کند یک
   سیگنالِ صادرشده کِی وارد پنل شود و کِی خارج — مثل یک تریدر، نه یک اندیکاتور.
   ورود: ۲ اسکن پیاپی SIGNAL + عدم تضاد شدید با BTC + نبود CHoCH مخالف در ۵م.
   خروج: فقط SL/TP2/فلیپ قوی/انقضا — یا ۳ اسکن پیاپیِ ضعیف (نه یک نوسان).
   ========================================================================= */
const STAB={confirmScans:2,weakScansToDrop:3,maxAgeMin:480,flipScore:S.strong_signal_score};
function fresh5mChochAgainst(r){
  try{
    const m5=r.metrics&&r.metrics["5m"];if(!m5)return false;
    if(r.direction==="LONG")return m5.direction==="SHORT"&&m5.macd_hist<0;
    if(r.direction==="SHORT")return m5.direction==="LONG"&&m5.macd_hist>0;
  }catch(e){}
  return false;
}
function btcConflicts(r,btcRow){
  if(!btcRow||r.binance_symbol==="BTCUSDT")return false;
  try{
    const b15=btcRow.metrics["15m"];
    if(r.direction==="LONG")return b15.direction==="SHORT"&&btcRow.direction==="SHORT"&&btcRow.score>=S.ready_score;
    if(r.direction==="SHORT")return b15.direction==="LONG"&&btcRow.direction==="LONG"&&btcRow.score>=S.ready_score;
  }catch(e){}
  return false;
}
function updateActiveSignals(rows){
  const now=Date.now();
  const btcRow=rows.find(x=>x.binance_symbol==="BTCUSDT")||null;
  const bySym={};rows.forEach(x=>bySym[x.binance_symbol]=x);
  const rawSignals=rows.filter(x=>x.status==="SIGNAL");
  const pending=STATE.pending||{};
  const stillActive=[];
  for(const a of (STATE.active||[])){
    const cur=bySym[a.binance_symbol];
    const price=cur?cur.entry:null;
    let close=null;
    if(price!=null){
      if(a.direction==="LONG"){
        if(price<=a.sl)close="STOPPED";
        else if(price>=a.tp2)close="TP2_HIT";
        else if(!a.tp1Hit&&price>=a.tp1){a.tp1Hit=true;a.events.push("TP1 ✓ "+fmtTime(new Date()));}
      }else{
        if(price>=a.sl)close="STOPPED";
        else if(price<=a.tp2)close="TP2_HIT";
        else if(!a.tp1Hit&&price<=a.tp1){a.tp1Hit=true;a.events.push("TP1 ✓ "+fmtTime(new Date()));}
    ...
    if(!close&&cur&&cur.status==="SIGNAL"&&cur.direction!==a.direction&&cur.score>=STAB.flipScore)close="FLIPPED";
    if(!close&&(now-a.bornAt)/60000>STAB.maxAgeMin)close="EXPIRED";
    if(close){
      a.closedAt=now;a.closeReason=close;a.lastPrice=price;
      STATE.closedSignals.unshift(a);STATE.closedSignals.splice(100);
      continue;
    }
    if(cur&&cur.status==="SIGNAL"&&cur.direction===a.direction){a.weak=0;a.score=cur.score;}
    else{a.weak=(a.weak||0)+1;}
    if(a.weak>=STAB.weakScansToDrop){
      a.closedAt=now;a.closeReason="WEAKENED";a.lastPrice=price;
      STATE.closedSignals.unshift(a);STATE.closedSignals.splice(100);
      continue;
    }
    a.lastPrice=price!=null?price:a.lastPrice;
    stillActive.push(a);
  }
  STATE.active=stillActive;
  const activeSyms=new Set(STATE.active.map(x=>x.binance_symbol));
  const nextPending={};
  for(const r of rawSignals){
    if(activeSyms.has(r.binance_symbol))continue;
    if(btcConflicts(r,btcRow)){nextPending[r.binance_symbol]={n:0,why:"تضاد با BTC — نگه‌داشته شد",r};continue;}
    if(domConflicts(r,STATE.market_context&&STATE.market_context.usdt_dom)){nextPending[r.binance_symbol]={n:0,why:"دامیننس تتر مخالف — نگه‌داشته شد",r};continue;}
    if(fresh5mChochAgainst(r)){nextPending[r.binance_symbol]={n:0,why:"ساختار ۵م مخالف — نگه‌داشته شد",r};continue;}
    const p=pending[r.binance_symbol];
    const n=(p&&p.n?p.n:0)+1;
    if(n>=STAB.confirmScans){
      STATE.active.push({
        binance_symbol:r.binance_symbol,asset_symbol:r.asset_symbol,direction:r.direction,
        setup:r.setup,score:r.score,entry:r.entry,sl:r.stop_loss,tp1:r.tp1,tp2:r.tp2,rr:r.rr,
        bornAt:now,weak:0,tp1Hit:false,lastPrice:r.entry,events:["صدور "+fmtTime(new Date())]
      });
    }else{
      nextPending[r.binance_symbol]={n,why:`تأیید ${n}/${STAB.confirmScans} — منتظر اسکن بعدی`,r};
    }
  }
  STATE.pending=nextPending;
  STATE.active.sort((a,b)=>b.bornAt-a.bornAt);
}
...
async function runScan(){
  ...
  try{
    ...
    hideProg();
    setStatus(`اسکن کامل شد · ${rows.length} نماد · ${STATE.active.length} سیگنال فعال · ${Object.keys(STATE.pending).length} در انتظار تأیید · ${STATE.watchlist.length} واچ‌لیست · ${fmtTime(STATE.last_scan_at)}`);
    renderAll();
...
function renderRegime(){
  ...
  const d=c.usdt_dom;
  const domCol=!d||!d.available?"var(--dim)":(d.dir==="UP"||d.dir==="UP_STRONG")?"var(--short)":(d.dir==="DOWN"||d.dir==="DOWN_STRONG")?"var(--long)":"var(--mut)";
  const domHtml=` · <span style="color:${domCol}">${domFa(d)}</span>`;
  $("#regimeTxt").innerHTML=`رژیم بازار: ${fa} `+(c.btc_dir?`· BTC ${c.btc_dir} ${c.btc_change_24h>=0?"+":""}${(c.btc_change_24h||0).toFixed(1)}%`:"")+domHtml;
}
...
function activeCard(a){
  const ageMin=Math.round((Date.now()-a.bornAt)/60000);
  const age=ageMin<60?ageMin+" دقیقه":Math.round(ageMin/60*10)/10+" ساعت";
  const chips=[
    `<span class="tag">${a.setup}</span>`,
    `<span class="tag">RR ${fmt(a.rr,2)}</span>`,
    `<span class="tag">عمر ${age}</span>`,
    a.tp1Hit?`<span class="tag" style="color:var(--long);border-color:#1c6b53">TP1 زده ✓ (SL→ورود)</span>`:"",
    (a.weak||0)>0?`<span class="tag warn">در حال تضعیف ${a.weak}/${STAB.weakScansToDrop}</span>`:`<span class="tag" style="color:var(--valid);border-color:#1c6b53">پایدار ✓</span>`,
  ].filter(Boolean).join("");
  const pl=a.lastPrice!=null?((a.direction==="LONG"?a.lastPrice/a.entry-1:1-a.lastPrice/a.entry)*100):null;
  return `<div class="card fade">
    <div class="top"><span class="sym">${a.asset_symbol}</span>
      <span class="side ${a.direction}">${a.direction}</span>
      <span class="statuspill st-SIGNAL" style="margin-inline-start:auto">فعال</span></div>
    <div class="scorebig">امتیاز صدور <b>${fmt(a.score,1)}</b>${pl!=null?` · حرکت از ورود <b style="color:${pl>=0?"var(--long)":"var(--short)"}">${pl>=0?"+":""}${pl.toFixed(2)}%</b>`:""}</div>
    <div class="levels">
      <div><div class="k">entry (قفل)</div><div class="val">${fmt(a.entry)}</div></div>
      <div><div class="k">stop</div><div class="val sl">${fmt(a.sl)}</div></div>
      <div><div class="k">tp1</div><div class="val tp">${fmt(a.tp1)}</div></div>
      <div><div class="k">tp2</div><div class="val tp">${fmt(a.tp2)}</div></div>
    </div>
    <div class="tags">${chips}</div>
    <div style="padding:0 14px 13px"><button class="btn ghost" style="width:100%" onclick='openDeep("${a.binance_symbol}")'>تحلیل کامل + الگو</button></div>
  </div>`;
}
const CLOSE_FA={STOPPED:"استاپ خورد",TP2_HIT:"TP2 کامل ✓",FLIPPED:"فلیپ جهت",EXPIRED:"منقضی (۸س)",WEAKENED:"تضعیف ۳اسکن"};
function renderSignals(){
  $("#cSig").textContent=STATE.active.length;
  const v=$("#view-signals");
  const pend=Object.entries(STATE.pending||{});
  const pendHtml=pend.length?`<div class="box" style="margin-bottom:12px"><h3>در انتظار تأیید (فیلتر پایداری)</h3><div class="candsig">`+
    pend.map(([s,p])=>`<span class="c ${p.r&&p.r.direction==="LONG"?"L":"S"}">${s.replace("USDT","")} · ${p.why}</span>`).join("")+
    `</div><p class="dim" style="font-size:11.5px;margin-top:8px">سیگنال فقط بعد از ${STAB.confirmScans} اسکن پیاپی + نبود تضاد BTC/ساختار ۵م وارد پنل می‌شود؛ بعد از ورود، با نوسان حذف نمی‌شود.</p></div>`:"";
  const closed=STATE.closedSignals.slice(0,8);
  const closedHtml=closed.length?`<div class="box" style="margin-top:12px"><h3>سیگنال‌های بسته‌شده (آخرین‌ها)</h3>`+
    closed.map(c=>`<div class="fbitem"><div class="t">${fmtTime(c.closedAt)}</div>
      <div><span class="s">${c.asset_symbol}</span> <span class="dim mono" style="font-size:11px">${c.direction} · ورود ${fmt(c.entry)}${c.tp1Hit?" · TP1✓":""}</span></div>
      <div class="fbverdict ${c.closeReason==="TP2_HIT"?"v-win":c.closeReason==="STOPPED"?"v-loss":"v-reject"}">${CLOSE_FA[c.closeReason]||c.closeReason}</div></div>`).join("")+`</div>`:"";
  v.innerHTML=`<div class="section-title"><span class="eyebrow">active · sticky</span><h2>سیگنال‌های فعال</h2></div>`+
    pendHtml+
    (STATE.active.length?`<div class="grid">${STATE.active.map(activeCard).join("")}</div>`:
    `<div class="empty">سیگنال فعالی نیست. سیگنال بعد از ${STAB.confirmScans} اسکن پیاپیِ تأییدشده وارد می‌شود و فقط با SL/TP2/فلیپ قوی/انقضا یا ${STAB.weakScansToDrop} اسکن ضعیف خارج می‌شود — نه با یک نوسان.</div>`)+
    closedHtml;
}
function watchRow(r){
  return `<div class="card fade">
